...
<template>
  <div class="mt-5 d-flex gap-1">
    <button @click="handleSortByName" class="btn btn-primary">Сортувати по імені <i class="bi"
                                                                              :class="sortedByNameAsc ? 'bi-sort-alpha-down' : 'bi-sort-alpha-up'"></i>
    </button>
    <button @click="handleSortByEmail" class="btn btn-primary">Сортувати по пошті <i class="bi"
  ...
  <div class="mt-3 p-4 mb-5 border border-1 border-secondary rounded bg-white">
    ...
    <VueModal id="deleteUser" :isOpen="isDeleteModalOpen" @close="closeDeleteModal">
      ...
      <template #body>
        <div>
          Ви впевнені що хочте видалити користувача: {{ userToDelete?.name }}, {{ userToDelete?.email }}
        </div>
      </template>
      <template #footer>
